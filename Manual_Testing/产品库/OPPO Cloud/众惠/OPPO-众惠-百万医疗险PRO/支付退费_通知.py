...
class Payment_refund:
    ...
    def Payment_refund(self):
        ...
        body = {
            "Data": {
                "AgencyPolicyRef": "TenSerPtlst0tG3K",  # 第三方渠道公司保单号码或者第三方渠道公司订单号码
                "PolicyRef": "P10E120240101V0014478",  # (非必填)保单号
                "InstallmentNo": "1",  # 分期号(兼容资金安全险非必填),非分期产品默认为第一期
                "TotalPremium": "1700.36",  # 保费，分期产品为对应期数保费
                "Currency": "CNY",  # 币别
                "PaymentMethod": "2",  # 支付方式：1-支付宝2-微信支付3-通联支付4-快钱支付
                "MerchantOrderNum": RandomStr().create(),  # (非必填)商户订单号（钱包侧给到微信/支付宝的订单号）
                "PaymentFlowNum": RandomStr().create(),
                # 说明：1、“支付流水号”为微信支付宝自动生成的（微信支付单号/支付宝交易号）2、“商户退款订单号”为钱包侧给到微信支付宝的（商户退款单号/退款请求号）
                "BusinessType": "2",  # 业务类型：1-支付，2-退保退费 , 3-批改退费
                "PayTime": Time(),  # 支付退费时间
                "PayStatus": "0",  # 支付/退费状态：0-成功1-失败
                "IsTest": "1",  # (非必填)是否测试单，只要是请求了钱包发生支付和退款时必传0-非测试单1-测试单(兼容资金安全险非必填)
            },
